...
struct PeopleView: View {
    ...
    private func gridView(_ geometry: GeometryProxy) -> some View {
        QGrid(Storage.people,
              ...
              hPadding: max(min(self.hPadding, self.hPaddingMax(geometry)), 0.0)) { item, _, _ in
            GridCell(person: item)
        }
    }
    ...
    private func hSpacingMax(_ geometry: GeometryProxy) -> CGFloat {
        return max(geometry.size.width / self.columns - 2 * self.hPadding, 1.0)
    }
